...
- **Nền `background-attachment: fixed` bị vẽ lại ở mọi màn chụp** — nền kiểu đó neo vào viewport chứ không neo vào trang, nên mỗi màn lại tô đúng một dải giống hệt; cùng loại lỗi với thanh app bar, chỉ khác tầng. Nay đổi tạm sang `scroll` trong lúc chụp rồi trả lại nguyên trạng. `html` và `body` được kiểm riêng vì `TreeWalker` gốc `body` không trả về chính `body`, mà đó lại là chỗ nền cố định hay nằm nhất. Có test hồi quy đếm số lần đổi màu của nền dọc ảnh: phải đúng một lần, và test đã được kiểm chứng là đỏ khi gỡ bản vá.
